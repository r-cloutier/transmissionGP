...
class GPEmulator(object):
    ...
    def __init__(self, samples, targets, lnhyperparams=None,
		 kernel='SE'):
	'''
	...
	Parameters
	----------
	`samples` : 2d array (Nsamples, Nparams)
	    Numpy array of sampled model parameters; i.e. input 
	    for the Gaussian process.
	`targets` : 1d (Nsamples,)
	    Numpy array of the target values; the PCs from projecting the 
	    simulator spectra onto their basis functions.
        `lnhyperparams` : array-like (Nparams + NGPhyperparams,)
            Optional list of log hyperparameters. If not given you will have to 
            learn the hyperparameters (see GPEumlator.learn_hyperparams).
	`kernel` : str
	    String corresponding to the type of assumed covariance kernel. 
            `kernel` must one of ['SE'].
	...
	self.Nsamples, self.Nparams = self.samples.shape
	self.kernel = kernel
	if lnhyperparams is not None:
	    self.lnhyperparams = \
                        np.array(lnhyperparams).reshape(len(lnhyperparams))
